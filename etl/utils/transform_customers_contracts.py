...
def transform_customers_contracts(df_cust_con, df_con, df_con_details, df_periods):
    ...
    try:
        ...
        df_copy_con_details = df_copy_con_details.merge(
            df_copy_periods, on="contract_period_id", how="left"
        )
        ...
        df_copy_con = df_copy_con.merge(
            df_copy_con_details, on="contract_details_id", how="left"
        )
        ...
        df_copy_cust_con = df_copy_cust_con.merge(
            df_copy_con, on="contract_id", how="left"
        )
        ...
        df_copy_cust_con["end_date"] = df_copy_cust_con.apply(
            lambda row: row["start_date"] + pd.DateOffset(months=row["period"]), axis=1
        )

        # checking if the contract is still active
        df_copy_cust_con["is_active"] = (
            df_copy_cust_con["end_date"] > pd.Timestamp.now()
        )

        # keeping only the wanted columns
        df_copy_cust_con = df_copy_cust_con[
            ["customer_id", "contract_id", "start_date", "end_date", "is_active"]
        ]
    except Exception as e:
        logger.error(
            f"failed to transform customers_contracts: {type(e).__name__}: {e}"
        )
