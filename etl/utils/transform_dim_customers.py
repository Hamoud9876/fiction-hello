...
def transform_dim_customers(df_customer, df_gender, df_pronounce, df_cust_sts):
    ...
    try:
        ...
        df_copy_customer = df_copy_customer.merge(
            df_copy_gender, on="gender_id", how="left"
        )
        df_copy_customer = df_copy_customer.merge(
            df_copy_pronounce, on="pronounce_id", how="left"
        )
        df_copy_customer = df_copy_customer.merge(
            df_copy_cust_sts, on="customer_status_id", how="left"
        )

        # making sure that it is date type as expected
        df_copy_customer["join_date"] = pd.to_datetime(
            df_copy_customer["join_date"]
        ).dt.date

        # renaming the column that were merged to match the distenation table
        df_copy_customer.rename(
            columns={
                "gender_title": "gender",
        ...
        df_copy_customer = df_copy_customer[
            [
                "join_date",
                "full_name",
                "gender",
