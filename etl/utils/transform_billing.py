...
def transform_billing(df_billing, df_billing_status):
    ...
    try:
        ...
        df_copy_billing["completed_date"] = pd.to_datetime(
            df_copy_billing["completed_date"]
        )
        df_copy_billing["due_date"] = pd.to_datetime(df_copy_billing["due_date"])

        # droping unwated columns to aviod clashing when merged
        df_copy_billing_status.drop(
            ["last_updated", "created_at"], axis=1, inplace=True
        )
        ...
        df_copy_billing = df_copy_billing.merge(
            df_copy_billing_status, on="bill_status_id", how="left"
        )
        ...
        df_copy_billing["is_paid"] = df_copy_billing.apply(
            lambda row: True if row["status"] == "paid" else False, axis=1
        )
        ...
        # renaming columns
        df_copy_billing.rename(
            columns={"status": "bill_status", "completed_date": "complete_date"},
            inplace=True,
        )

        # dropping unwated columns
        df_copy_billing = df_copy_billing[
            [
                "customer_id",
                "contract_id",
                "amount_billed",
