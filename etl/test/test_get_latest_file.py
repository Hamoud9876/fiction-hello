...
class TestIntegerationLatestFile:
    def test_returns_df(self, aws_credentials, s3_client):
        ...
        for i in range(2):
            ...
            my_client.put_object(Body=b"content,hello", Bucket=bucket_name, Key=s3_key)

        response = get_latest_file("Hi", bucket_name)
    ...
    def test_returns_correct_content(self, aws_credentials, s3_client):
        ...
        csv_content = (
            b"content,hello,created_at,last_updated\n1,2,2025-09-14,2025-09-14\n"
        )

        prefix = f"Hi/{today.year}/{today.month}/{today.day}"

        for i in range(2):
            s3_key = f"{prefix}/{i}-12-12-12.csv"
            my_client.put_object(Body=csv_content, Bucket=bucket_name, Key=s3_key)
