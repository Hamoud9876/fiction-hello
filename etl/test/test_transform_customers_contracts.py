...
def create_data():
    contract_periods = pd.DataFrame(
        {"contract_period_id": range(1, 4), "period": [12, 24, 36]}
    )
    contract_periods["created_at"] = [
        fake.date_time_between(start_date="-2y", end_date="now") for _ in range(3)
    ]
    contract_periods["last_updated"] = contract_periods["created_at"] + pd.to_timedelta(
        np.random.randint(0, 365, size=3), unit="d"
    )
...
class TestTransformCustomersContracts:
    def test_returns_df(self, create_data):
        ...
        response = transform_customers_contracts(
            df_cust_con, df_con, df_con_detail, df_periods
        )
