...
def create_data():
    ...
    billing["completed_date"] = [
        (
            date + timedelta(days=np.random.randint(0, 31))
    ...
    billing["created_at"] = [
        fake.date_time_between(start_date="-2y", end_date="now") for _ in range(n)
    ]
    billing["last_updated"] = billing["created_at"] + pd.to_timedelta(
        np.random.randint(0, 365, size=n), unit="d"
    )

    billing_status = pd.DataFrame(
        {"bill_status_id": [1, 2], "status": ["paid", "unpaid"]}
    )
    billing_status["created_at"] = [
        fake.date_time_between(start_date="-2y", end_date="now") for _ in range(2)
    ]
    billing_status["last_updated"] = billing["created_at"] + pd.to_timedelta(
        np.random.randint(0, 365, size=n), unit="d"
    )
